...
public class AnnotationBean extends AbstractConfig implements DisposableBean, BeanFactoryPostProcessor, BeanPostProcessor, ApplicationContextAware {
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName)
        ...
        if (service != null) {
            ...
            //modified by lubin.xuan start
            //添加方法级超时设置注解支持
            Method[] methods = clazz.getMethods();
            if (null != methods && methods.length > 0) {
                Map<String, MethodConfig> methodConfigMap = new HashMap<>();
                for (Method method : methods) {
                    com.alibaba.dubbo.config.annotation.Method config = method.getAnnotation(com.alibaba.dubbo.config.annotation.Method.class);
                    if (null != config) {
                        MethodConfig methodConfig = new MethodConfig();
                        methodConfig.setName(method.getName());
                        int set = 0;
                        if (config.timeout() > 0) {
                            methodConfig.setTimeout(config.timeout());
                            set++;
                        }
                        if (config.actives() > 0) {
                            methodConfig.setActives(config.actives());
                            set++;
                        }
                        if (config.retries() > 0) {
                            methodConfig.setRetries(config.retries());
                            set++;
                        }
                        if (set > 0) {
                            methodConfigMap.put(methodConfig.getName(), methodConfig);
                        }
                    }
                }
                ...
                if (null != methodConfigs && !methodConfigs.isEmpty()) {
                    for (MethodConfig methodConfig : methodConfigs) {
                        methodConfigMap.remove(methodConfig.getName());
                    }
                }
                if (!methodConfigMap.isEmpty()) {
                    if(null==serviceConfig.getMethods()){
                        serviceConfig.setMethods(new ArrayList<>());
                    }
                    serviceConfig.getMethods().addAll(methodConfigMap.values());
                }
            }
            //modified by lubin.xuan end
