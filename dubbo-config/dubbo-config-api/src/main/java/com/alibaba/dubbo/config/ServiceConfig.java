...
public class ServiceConfig<T> extends AbstractServiceConfig {
    ...
    private void doExportUrlsFor1Protocol(ProtocolConfig protocolConfig, List<URL> registryURLs) {
        ...
        //modified by Lubin.Xuan  Desc. 设置默认版本号
        if(null==map.get(Constants.VERSION_KEY)||map.get(Constants.VERSION_KEY).length()<1) {
            String version2set = map.get("revision");
            if(null==version2set||version2set.trim().length()<1){
                version2set = this.application.getVersion();
            }
            ...
            map.put(Constants.VERSION_KEY, version2set);
        }
        //modified by Lubin.Xuan
