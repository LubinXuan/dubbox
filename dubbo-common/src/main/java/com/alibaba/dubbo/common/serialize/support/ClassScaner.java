...
/**
 * Created by Lubin.Xuan on 2015/3/24.
 * ie.
 */
public class ClassScaner {
     ...
     */
    public ClassScaner(String[] packagesToScan, Class<? extends Annotation>... annotationFilter) {
                ...
                for (Resource resource : resources) {
                    if (resource.isReadable()) {
                        ...
                        if (matchesEntityTypeFilter(reader, readerFactory)) {
                            ...
                            if (!validSerializableClass(clazz)) {
                                continue;
                            }
    ...
    private boolean validSerializableClass(Class clazz) {

        if (!Serializable.class.isAssignableFrom(clazz)) {
            logger.warn("不是Serializable的Class   " + clazz);
            return false;
        }

        if (clazz.isInterface() || clazz.isAnnotation()) {
            logger.warn("isInterface  isAnnotation  " + clazz);
            return false;
        }
