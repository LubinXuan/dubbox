...
//        System.out.println(">>>>> Total time consumed:" + (System.currentTimeMillis() - start));
        ...
        System.out.println("anotherUserRestService SUCCESS: registered user with id " + anotherUserRestService.registerUser(user).getId());

        RpcContext.getContext().setAttachment("clientName", "demo");
        RpcContext.getContext().setAttachment("clientImpl", "dubbox");
        System.out.println("anotherUserRestService SUCCESS: got user " + anotherUserRestService.getUser(1L));

        System.out.println(userRestService.getUser(1l));
    }
